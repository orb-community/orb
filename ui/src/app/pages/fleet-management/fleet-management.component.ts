...
export class FleetManagementComponent implements OnInit {
  ...
  openAddModal() {
    this.dialogService.open(FleetManagementAddComponent, { context: { action: 'Create' } }).onClose.subscribe(
      confirm => {
        if (confirm) {
          this.getGroups();
        }
  ...
  openEditModal(row: any) {
    this.dialogService.open(FleetManagementAddComponent, { context: { formData: row, action: 'Edit' } }).onClose.subscribe(
      confirm => {
        if (confirm) {
          this.getGroups();
        }
