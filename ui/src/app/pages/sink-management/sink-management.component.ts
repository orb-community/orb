...
import { SinkManagementAddComponent } from 'app/pages/sink-management/add/sink-management.add.component';

const defFreq: number = 100;

@Component({
  selector: 'ngx-sink-management-component',
  templateUrl: './sink-management.component.html',
  styleUrls: ['./sink-management.component.scss'],
})
export class SinkManagementComponent implements OnInit {
  ...
  openAddModal() {
    this.dialogService.open(SinkManagementAddComponent, { context: { action: 'Create' } }).onClose.subscribe(
      confirm => {
        if (confirm) {
          this.getGroups();
        }
  ...
  openEditModal(row: any) {
    this.dialogService.open(SinkManagementAddComponent, { context: { formData: row, action: 'Edit' } }).onClose.subscribe(
      confirm => {
        if (confirm) {
          this.getGroups();
        }
