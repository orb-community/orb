...
export class FleetsComponent implements OnInit {
  ...
  openAddModal() {
    this.dialogService.open(FleetsAddComponent, {context: {action: 'Create'}}).onClose.subscribe(
      confirm => {
        if (confirm) {
          this.getGroups();
        }
  ...
  openEditModal(row: any) {
    this.dialogService.open(FleetsAddComponent, {context: {formData: row, action: 'Edit'}}).onClose.subscribe(
      confirm => {
        if (confirm) {
          this.getGroups();
        }
