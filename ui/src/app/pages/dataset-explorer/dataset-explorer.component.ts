...
@Component({
  ...
  styleUrls: ['./dataset-explorer.component.scss'],
})
export class DatasetExplorerComponent implements OnInit {
  ...
  openAddModal() {
    this.dialogService.open(DatasetExplorerAddComponent, { context: { action: 'Create' } }).onClose.subscribe(
      confirm => {
        if (confirm) {
          this.getGroups();
        }
  ...
  openEditModal(row: any) {
    this.dialogService.open(DatasetExplorerAddComponent, { context: { formData: row, action: 'Edit' } }).onClose.subscribe(
      confirm => {
        if (confirm) {
          this.getGroups();
        }
