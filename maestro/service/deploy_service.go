...
func (d *eventService) HandleSinkActivity(ctx context.Context, event maestroredis.SinkerUpdateEvent) error {
	...
	// check if exists deployment entry from postgres
	_, _, err := d.deploymentService.GetDeployment(ctx, event.Owner, event.SinkID)
	if err != nil {
		d.logger.Error("error trying to get deployment entry", zap.Error(err))
		return err
	}
	...
	_, err = d.deploymentService.NotifyCollector(ctx, event.Owner, event.SinkID, "deploy", "", "")
	if err != nil {
		d.logger.Error("error trying to notify collector", zap.Error(err))
		return err
	}
	err2 := d.deploymentService.UpdateStatus(ctx, event.Owner, event.SinkID, "provisioning_error", err.Error())
	if err2 != nil {
...
func (d *eventService) HandleSinkIdle(ctx context.Context, event maestroredis.SinkerUpdateEvent) error {
	...
	go func() {
		_ = d.deploymentService.UpdateStatus(ctx, event.Owner, event.SinkID, "idle", "")
	}()
	_, err = d.deploymentService.NotifyCollector(ctx, event.Owner, event.SinkID, "deploy", "", "")
	if err != nil {
		d.logger.Error("error trying to notify collector", zap.Error(err))
