...
type DeploymentListenerController interface {
	...
	// CreateDeploymentEntry - when a sink is created, create a deployment entry in database and redis with its configuration
	CreateDeploymentEntry(ctx context.Context, sink config.SinkData) error
	// GetDeploymentEntryFromSinkId - get the deployment entry from the sink id
...
func (es eventStore) handleSinksUpdateCollector(ctx context.Context, event maestroredis.SinksUpdateEvent) error {
...
func (ls *sinksListenerService) handleSinksDeleteCollector(ctx context.Context, event maestroredis.SinksUpdateEvent) error {
	...
	err := ls.RemoveSinkActivity(ctx, event.SinkID)
	if err != nil {
		return err
	}

	deploymentEntry, err := ls.deploymentService.GetDeployment(ctx, event.Owner, event.SinkID)
	if err != nil {
		ls.logger.Error("did not find collector entry for sink", zap.String("sink-id", event.SinkID))
		return err
	}
	...
	err = ls.sinkerKeyRedisClient.HDel(ctx, deploymentKey, event.SinkID).Err()
	if err != nil {
		return err
	}
	err = ls.kubecontrol.DeleteOtelCollector(ctx, event.Owner, event.SinkID, deploymentEntry)
	if err != nil {
		return err
	}
	return nil
