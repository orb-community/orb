...
Feature: agent provider
    ...
    Scenario: Apply two policies to an agent
        Given the Orb user logs in
            ...
            And that container logs contains messages "scraped metrics for policy", "policy applied successfully" and "managing agent policy from core" for applied policy within 180 seconds
        When a new policy is created
            And a new dataset is created using referred group, sink and policy ID
        Then the container logs should contain messages "scraped metrics for policy", "policy applied successfully" and "managing agent policy from core" for applied policy within 180 seconds
