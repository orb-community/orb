...
def run_agent_config_file(agent_name, overwrite_default=False, only_file=False, port=10583, config_file_path="/opt/orb",
    ...
    if overwrite_default is True:
        command = f"docker run -d -v {volume} -p {port}:{port} --net=bridge {agent_image}"
    else:
        command = f"docker run -d -v {volume} -p {port}:{port} --net=bridge {agent_image} run -c {agent_command}"
    args = shlex.split(command)
