...
Scenario: Sink with invalid endpoint
    ...
    When a new dataset is created using referred group, policy and 1 sink ID
...
Scenario: Edit an advanced policy with handler dns changing the handler to net
    Given the Orb user has a registered account
        ...
        And a new dataset is created using referred group, policy and 1 sink ID
...
Scenario: remove 1 sink from a dataset with 2 sinks
    Given the Orb user has a registered account
        ...
        And an Agent Group is created with same tag as the agent
        And a new policy is created using: handler=dhcp
        And a new dataset is created using referred group, policy and 2 sinks ID
...
Scenario: remove 1 sink from a dataset with 1 sinks
    Given the Orb user has a registered account
        ...
        And an Agent Group is created with same tag as the agent
        And a new policy is created using: handler=dhcp
        And a new dataset is created using referred group, policy and 1 sinks ID
    When remove 1 of the linked sinks from orb
